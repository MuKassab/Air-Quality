...
export const validate = schema => {
  ...
  return (req, res, next) => {
    if (!_.isEmpty(params)) {
      const { value: validatedParams, error } = compliedParamsSchema.validate(
        req.params,
        options,
      );
      ...
      if (!_.isNil(error)) {
        throw new CustomAPIError({
          message: ERROR_CODES.PARAMS_VALIDATION_FAILED.message,
          status: BAD_REQUEST,
          errorCode: ERROR_CODES.PARAMS_VALIDATION_FAILED.errorCode,
    ...
    if (!_.isEmpty(body)) {
      const { value: validatedBody, error } = compliedBodySchema.validate(
        req.body,
        options,
      );
      ...
      if (!_.isNil(error)) {
        throw new CustomAPIError({
          message: ERROR_CODES.BODY_VALIDATION_FAILED.message,
          status: BAD_REQUEST,
          errorCode: ERROR_CODES.BODY_VALIDATION_FAILED.errorCode,
          meta: { errors: extractValidationErrors(error) },
        });
      }
    ...
    if (!_.isEmpty(query)) {
      const { value: validatedQuery, error } = compliedQuerySchema.validate(
        req.query,
        options,
      );
      ...
      if (!_.isNil(error)) {
        throw new CustomAPIError({
          message: ERROR_CODES.QUERY_VALIDATION_FAILED.message,
          status: BAD_REQUEST,
          errorCode: ERROR_CODES.QUERY_VALIDATION_FAILED.errorCode,
          meta: { errors: extractValidationErrors(error) },
        });
      }
